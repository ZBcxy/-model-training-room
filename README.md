...
| 实验 | 数据量 | 数据质量 | 最终 Loss | 耗时 | 效果 |
|------|--------|----------|-----------|------|------|
| v0.1 | 500 条 | ❌ 混搭模板 | 2.09 | 1.8 分钟 | 答非所问 |
| v0.2 | 500 条 | ✅ 手工匹配 | 0.11 | 2.1 分钟 | 准确但窄 |
| **v1.0** | **10,310 条** | **✅ 真实标注** | **1.12** | **69 分钟** | **准确丰富** |

**核心发现：数据质量 > 数据数量 > 模型大小。** LoRA 微调仅需 4GB 显存，入门显卡也能用。
